...
inline void UpdateBlobField(const Schema* schema,
                            const _detail::FieldExtractor* fe,  // field extractor
    ...
    schema->ParseAndSetBlob(record, data, [&](const Value& v) { return bm->Add(txn, v); }, fe);
}
...
EdgeIndexIterator Transaction::GetEdgePairUniqueIndexIterator(size_t label_id, size_t field_id,
                                                              VertexId src_vid, VertexId dst_vid,
                                                              const FieldData& key_start,
                                                              const FieldData& key_end) {
    EdgeIndex* index = GetEdgeIndex(label_id, field_id);
